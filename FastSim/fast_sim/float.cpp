...
template<>
void FastSim<gpu, float>::operator ()(const int &start_pos, const int &N_batch, float fee){
    assert(start_pos + N_batch <= N_samp);
    dvf dev_A = stgy, dev_B(N_feat * N_batch), dev_C(N_stgy * N_batch);
    ...
    t_end = clock();
    cout<<"Time usage for matrix multiplication is "<<float(t_end - t_start)/CLOCKS_PER_SEC<<" s"<<endl;
    
    // Initialization of GPU memories
    t_start = clock();
    dvf dev_mid(N_batch), dev_gap(N_batch), dev_prof = prof, dev_prc = last_prc;
    ...
    t_end = clock();
    cout<<"Time usage for running simulation is "<<float(t_end - t_start)/CLOCKS_PER_SEC<<" s"<<endl;
    return;
}

template <>
void FastSim<gpu, float>::fastSimulation(const vector<vector<float>> &weights, const vector<int> &late, const int &N_batch, float fee){
...
int main(int argc, char *argv[]){
    ...
    FastSim<gpu, float> test(signals, prices);
    clock_t t_end = clock();
    cout<<"Time usage for reading the data is "<<float(t_end - t_start)/CLOCKS_PER_SEC<<" s"<<endl<<endl;
    ...
    cout<<"Time usage for generating the weights is "<<float(t_end - t_start)/CLOCKS_PER_SEC<<" s"<<endl<<endl;
    
    t_start = clock();
    test.fastSimulation(weights, late, 200000, float(0.008));
    t_end = clock();
    cout<<"Time usage for gpu fast sim is "<<float(t_end - t_start)/CLOCKS_PER_SEC<<" s"<<endl<<endl;
